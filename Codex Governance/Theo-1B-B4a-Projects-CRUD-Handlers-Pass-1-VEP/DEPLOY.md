...
1. **Migration.** Run `b4a_migration.sql` against `vaultgpt-postgres-prod` (adds the `theo_projects.description` column, NOT NULL DEFAULT ''; existing rows backfill to ''; inherits existing RLS policies).
