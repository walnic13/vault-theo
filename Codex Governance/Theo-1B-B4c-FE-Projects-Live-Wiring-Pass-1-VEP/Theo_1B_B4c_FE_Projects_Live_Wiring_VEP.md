...
| **G-3** | **`theo_list_projects` omits knowledge.** The list endpoint returns project rows without `knowledge[]` (B4a decision). | **PROCEED** — `openProject` lazy-loads knowledge via `theo_list_project_knowledge`; "Start a chat in this project" is reachable only from the open detail, so knowledge is loaded before injection. |
