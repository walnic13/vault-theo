...
Pass: Pass 1
Sub-phase Track: N/A
Detail: **Codex-fix revision** (Pass-2 REJECT round 1): a cited assistant turn that also emits an artifact used to feed the `\0A:<id>\0` placeholder into `CitedText` → `Formatted` (markdown), which never runs `splitAssistant` — so the artifact card wouldn't render (raw placeholder shown). Fixed by giving `CitedText` an optional `renderText` render-prop (defaults to `Formatted`) and having `ChatView` pass `renderAssistant` (which does `splitAssistant` → `ArtifactCard`); a cited-and-artifact turn now renders both the card and the citation chips. Pre-existing latent path (send already put `display` into `CitedRun.text`); this closes it. Pass 1 — Frontend Verified Evidence Pack. **Visual-authority DEVIATION** turn (minor; like B4e–B4g): `ArtifactsView` now renders persisted-artifact **summaries** (metadata: type/version/updated) rather than in-memory `Artifact`s with a body preview (the gallery list carries no content — content is fetched on open). Per FE Governor a planned visual deviation is valid when **cited + classified (VISUAL-AUTHORITY-DEVIATION) + Rule-Anchored**, with **Walter** as the exemption authority — he directed artifacts persistence this session ("let's knock out artifacts persistence"; "the artifacts section doesn't seem to work"). VA-T1 read this turn (`frontend/theo-frontend-reference.jsx` artifacts region). Consumed endpoints are DEPLOYED + golden-verified (B4h `9c9c4d8`) and documented in API-Spec §2.3 (landed `db79681`). The design fact that reopened chats can re-derive artifacts client-side was **verified this turn by read-only SQL** (13/71 assistant turns retain the raw `[[ARTIFACT]]` markers in `theo_messages.content`). The ten proposed files were applied to a scratch copy of `src` this turn and pass `npm run typecheck` (exit 0) + `eslint .` (exit 0) + `npm run build` (TheoSurface 237.03 KB / 70.46 KB gzip); `src` reverted so the package carries only `proposed-src/`.
...
| 15 | ACTIVE (modify — Codex fix) — `src/theo/components/CitedText.tsx` (VA-T5) | `Read(full)` this turn | `7ed14f6966ae017359ac4066c61b136eaf84dd35` |
| 16 | ACTIVE (modify — Codex fix) — `src/theo/components/ChatView.tsx` | `Read(full)` this turn | `55a09d32f89d5556afdbd40a5a445fc677f8a848` |
| 17 | **Reused-unchanged** — `src/theo/lib/artifacts.ts` (parseArtifacts/upsert/remapToIds) | `Read(full)` this turn | `7d08753015398d23be0de6704e59d7c72aeb7c06` |
| 18 | **VA-T1 artifact** (registered §4B) — `frontend/theo-frontend-reference.jsx` (artifacts region) | `Grep("Artifacts\|ARTIFACT")` this turn | `433f6236344f6e8bdbc49db85a53036427610fed` |
...
| VA-T5 (`CitedText`) — Codex round-1 fix | `CitedText` gains an optional `renderText?: (text: string) => ReactNode` prop, defaulting to the existing `Formatted` — so a plain cited answer renders **byte-identically** to before. `ChatView` passes `renderAssistant` (which does `splitAssistant` → `ArtifactCard`), so a cited turn that also emits an artifact renders the card + the citation chips (previously the raw `\0A:<id>\0` placeholder hit markdown). Citation-chip rendering, favicon/source-card behaviour, and index numbering are untouched. | VISUAL-AUTHORITY-MATCH (additive render-prop; default preserves VA-T5 exactly) |
...
No un-cited deviation. No browser→model call. `ArtifactCard`, `ArtifactPanel`, `Sidebar`, `ProjectsView`, `ProjectDetail`, `ui.tsx`, `lib/artifacts.ts` unchanged (`ChatView`/`CitedText` touched only for the cited-artifact render fix above).
...
| **G-4** | **Re-derive + citations (Codex round-1 fix).** A cited assistant turn that also emits an artifact carries the `\0A:<id>\0` placeholder in `CitedRun.text`; `CitedText` renders that text via `Formatted` (markdown), which does not run `splitAssistant`. | **PROCEED (fixed)** — `CitedText` now takes a `renderText` render-prop and `ChatView` passes `renderAssistant`, so the cited path splits artifact placeholders → `ArtifactCard`. Applies to both live send and reload; default `renderText` keeps plain cited answers unchanged. |
...
| TC-9 | `components/CitedText` (**ACTIVE**, modify — Codex round-1 fix) | `export function CitedText({ runs, startIndex = 1, renderText }: { runs: CitedRun[]; startIndex?: number; renderText?: (text: string) => ReactNode })`. Adds `const body = (text) => renderText ? renderText(text) : <Formatted text={text} />`; both branches render `body(run.text)` (was `<Formatted text={run.text} />`). Citation-chip row, `CitationMarker`, favicon/source-card, index numbering **unchanged**. Default (no `renderText`) is byte-identical to the prior render. | VA-T5 → MATCH (additive optional prop; default preserves VA-T5) | none (pure UI) | PROCEED |
| TC-10 | `components/ChatView` (**ACTIVE**, modify — Codex round-1 fix) | Cited-assistant render becomes `<CitedText runs={m.runs} renderText={renderAssistant} />` (was `<CitedText runs={m.runs} />`). `ChatViewProps` (incl. `renderAssistant: (content: string) => ReactNode`) **unchanged**; no other change. | VA-T1 (chat surface) | via `renderAssistant` | PROCEED |

**Infra:** consumes the already-baked `VITE_FUNCTIONS_URL`. No `vite.config`/dependency change. `ArtifactCard.tsx`, `ArtifactPanel.tsx`, `Sidebar.tsx`, `ProjectsView.tsx`, `ProjectDetail.tsx`, `RowManage.tsx`, `Customize.tsx`, `ui.tsx`, `icons.tsx`, `lib/artifacts.ts`, `swapBlock.ts` unchanged.

## F-P6 — Repository & active-surface grounding
Targets read this turn, ACTIVE @ vault-theo `db79681`: `types.ts`, `useTheoState.ts`, `TheoSurface.tsx`, `services/theoClient.ts` + `gateway.live.ts` + `gateway.mock.ts`, `components/ArtifactsView.tsx` + `TheoMain.tsx` + `CitedText.tsx` + `ChatView.tsx` (all in the GCR with blob SHAs); reused-unchanged `lib/artifacts.ts`. Guardrails: gateway abstraction preserved (persist/list/get route through `theoClient`); no browser→model call; **no `localStorage`/`sessionStorage`** (`galleryArtifacts` is React state, the in-memory working set lives in `theoClient`); no Tailwind; no `reporting_*`. Design fact (reopened-chat re-derivation) grounded by read-only SQL this turn.

## F-P7 — VEP assembly
GCR (§3) + Rule Anchors (§5) open the pack; F-P1→F-P6 walked; the visual deviation is cited + classified + anchored + Walter-authorized (F-P2); Gap Disclosure present (G-1…G-4 all PROCEED; G-4 = the Codex round-1 cited-artifact render fix); CCT locked (10 rows, full interfaces, TheoMain/TheoSurface/ChatView interfaces pasted though unchanged). Codex-fix revision: `CitedText` gains a `renderText` render-prop, `ChatView` passes `renderAssistant` (TC-9/TC-10). Validated this turn (`tsc` + `eslint .` exit 0 + `build` green; TheoSurface 237.03 KB / 70.46 KB gzip; `src` reverted). On Codex APPROVAL, Pass 3 commits the eight files and Walter redeploys the Theo SWA → **artifacts persist**: generate one in chat (it saves), reopen the chat (its card is back), open the Artifacts tab (it's listed across chats), click it (versions + content load from Blob). Walter SWA acceptance = Visual Acceptance Evidence.
