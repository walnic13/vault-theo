...
Pass: Pass 1
Sub-phase Track: N/A
Detail: **Codex-fix revision (Pass-2 REJECT round 1):** the badge read `Project.memberCount` from the held `projects` list, but the live invite/revoke path (`shareMember`/`unshareMember`) refreshed only `projectMembersState` (the detail's member list), never `projects`, and returning to the grid (`go("projects")`) does not reload projects (`loadProjects` is mount-driven) — so after the exact invite workflow this microstep targets, the owner returned to the grid and still saw no badge. Fixed by updating the held `projects[].memberCount` on every share/unshare settle from the authoritative re-list count (`setProjects(ps => ps.map(p => p.id === projectId ? { ...p, memberCount: members.length } : p))`) in all four re-list blocks — the same optimistic-list discipline `setProjectVisibility` already uses. This pulls `useTheoState.ts` into the microstep (5 files). Pass 1 — Frontend Verified Evidence Pack. **Visual-authority DEVIATION** turn (extends the existing B5a/B5c sharing badge): the owner's grid badge now also fires for a privately-shared project (`memberCount > 0`), reusing the same pill chrome/tokens. Per FE Governor a planned visual deviation is valid when **cited + classified (VISUAL-AUTHORITY-DEVIATION) + Rule-Anchored**, with **Walter** as the exemption authority — he flagged the missing owner badge on the SWA this session. VA-T1 read this turn (`frontend/theo-frontend-reference.jsx` project region). Consumed contract deployed + golden-verified (`theo_list_projects` owner-gated `member_count`, B5d `f3868e6`; owner private+invite → `member_count=1`, owner private/none → 0, group → 0, non-owner row → 0 no-leak). The four proposed files were applied to `src` this turn and pass `npm run typecheck` (exit 0) + `eslint` (exit 0) + `npm run build` (TheoSurface 247.38 KB / 72.65 KB gzip); `src` reverted so the package carries only `proposed-src/`.
...
| 10a | ACTIVE (modify — R1) — `src/theo/useTheoState.ts` (share/unshare update held `projects[].memberCount`) | `Read` + `grep` this turn | `2e03b65b926a996a04a260b1be300719dac34e79` |
...
| **G-4** | **Grid reflects invite/revoke without remount (Codex R1 fix)** — `shareMember`/`unshareMember` now update the held `projects[].memberCount` on settle (authoritative re-list count), so the owner returning to the grid via `go("projects")` sees the badge immediately; `loadProjects` (mount) remains the source of truth on reload. | **PROCEED (fixed)** — optimistic-list update, same discipline as `setProjectVisibility`. |
...
| TC-5 | `useTheoState` (**ACTIVE**, modify — R1 state owner) | In `shareMember`/`unshareMember`, each of the four member re-list blocks (success + catch of both) additionally updates the held projects list so the grid badge reflects the change without a remount: after `const members = await theoClient.listProjectMembers(projectId)`, adds `setProjects((ps) => ps.map((p) => (p.id === projectId ? { ...p, memberCount: members.length } : p)))` (authoritative count from the re-list; the keyed `setProjectMembersState` guard is unchanged). No signature/exposure change. | VA-T1 (state owner) | `theo_list_project_members` count → `Project.memberCount` | PROCEED |
...
## F-P7 — VEP assembly
GCR (§3) + Rule Anchors (§5) open the pack; F-P1→F-P6 walked; the visual deviation is cited + classified + anchored + Walter-authorized (F-P2); Gap Disclosure present (G-1…G-4 all PROCEED); CCT locked (5 rows, full interfaces, ProjectsView interface noted unchanged). Validated this turn (`tsc` + `eslint` exit 0 + `build` green; TheoSurface 247.38 KB / 72.65 KB gzip; `src` reverted). On Codex APPROVAL, Pass 3 commits the four files and Walter redeploys the Theo SWA → an owner's privately-shared project now badges **"Shared"** on the grid (closing Walter's observed gap); non-owner badges unchanged. Walter SWA acceptance = Visual Acceptance Evidence.
