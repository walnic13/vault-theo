...
| 9 | **Contract basis (2/2)** — Theo Azure Postgres Schema — `spec/THEO_AZURE_POSTGRES_SCHEMA.md` (§3 set; §7 `theo_attachments` DEPLOYED row pending G-2) | `Grep("Four separate policies per table")` this turn | `95538419e01ff95d20342e6b65d97c332912c614` |
...
| G-4 | **Schema-doc §7 `theo_attachments` DEPLOYED row.** The B8a G-2 Role-C is authored (`Theo-1B-B8a-Schema-Doc-DEPLOYED-RoleC`) but **pending Codex execution** — the schema doc does not yet carry the row. | **PROCEED** — does not block B8b; the contract basis is the RO-verified B8a migration (doc #8). Codex should execute the pending G-2 Role-C. |
