# Theo 1B — B8d MI-Endpoint Protocol Fix (`theo_message`) — Pass 1 Backend VEP

> Pipeline: Vault Theo backend regime. Author = Claude Code (Pass 1). Reviewer = **Codex** (Pass 2). Plan-only; complete corrected handler for Walter to **redeploy `theo_message` only** at Pass 3, after which Claude Code re-runs the B8d payoff golden curls. **Microstep:** a golden-curl-stage fix to the **deployed** B8d `theo_message` (blob `da15fe3a`). The attachment injection path calls the managed-identity token endpoint to read blobs, but `theo_message`'s `requestUrl` is hardcoded to **`https.request`**, while the MI `IDENTITY_ENDPOINT` is an **`http://` localhost sidecar** — so the token fetch throws and every attachment degrades to a "could not be loaded" note (regression + negatives were unaffected). The fix makes `requestUrl` (and `requestBinary`) **protocol-aware**, an **EXACT mirror** of the deployed B8c `requestUrl`. **Scope: `theo_message` only.**
...
Turn issued against HEAD: `cdbb892b4b589916de73c0599c63a0c19baf534f` (vault-theo, `development`)
Grounding Mode: Full Baseline Grounding
Pass: Pass 1
Sub-phase Track: P8
Detail: Pass 1 backend fix VEP; P1–P8 walked. Artifact under fix = the deployed B8d `theo_message.index.js` (blob `da15fe3a`). Root cause proven at Pass-3 golden-curl time: both attachment payoff tests returned the `buildAttachmentBlocks` top-level catch note ("could not be loaded"), which fires when `getManagedIdentityAccessToken` throws; `theo_message`'s `requestUrl` is hardcoded `https.request`, but the MI `IDENTITY_ENDPOINT` is `http://` — so the token GET over HTTPS fails. The deployed B8b/B8c handlers read blobs via the SAME managed identity and work because their `requestUrl` is protocol-aware (`const lib = url.protocol === "http:" ? http : https`). Fix = add `const http = require("http")` + make `requestUrl`/`requestBinary` select the lib by `url.protocol` (3-line delta) — an **EXACT mirror** of the deployed B8c `requestUrl` (Golden §3; no new-domain helper). All non-MI calls (Foundry, token, embed, search, blob) are HTTPS and unaffected; regression + negatives already passed. Full Baseline per Conformance §4.
...
| 4 | Theo Golden Handler Standard — `governance/THEO_GOLDEN_HANDLER_STANDARD.md` (§3 EXACT-mirror helper) | `Grep("EXACT mirror against a deployed handler")` this turn | `f9123546b6b1112a9c0c7e66ef776b2c75751593` |
...
| 8 | **Artifact under fix** — deployed B8d gateway — `Codex Governance/Theo-1B-B8d-Gateway-Attachment-Injection-Pass-1-VEP/theo_message.index.js` | `Read(full)` this turn | `da15fe3ac55ae32bfd1262e7ae3801c0eadf60b4` |
| 9 | **Primary Reference function.json** — `api/theo_message/function.json` | `Read(full)` this turn | `bd476fc8d144ed9592b561b4c0ded84f5911cff0` |
| 10 | **EXACT-mirror precedent (protocol-aware requestUrl)** — deployed B8c finalize — `Codex Governance/Theo-1B-B8c-Attachment-Extraction-Pass-1-VEP/theo_finalize_attachment.index.js` | `Grep("url.protocol === \"http:\"")` this turn | `b829d98d0ad93e03f61f6ef56199c92f9d80b37c` |

No ChatGPT advisory cited (§4D / T18). No `reporting_*`/`corporate-reporting`/`axis` change. No write SQL (plan only). No schema/dependency/env change. Token bytes never printed.
...
| governance/THEO_PHASE_1B_BACKEND_PLAN.md | Tier B8 | "gateway integration" | §P1 — the B8d gateway under fix |
| governance/THEO_GOLDEN_HANDLER_STANDARD.md | §3 | "an EXACT mirror against a deployed handler containing that helper" | §CHANGESET — protocol-aware requestUrl mirrors deployed B8c |

---

## P1 — Feature identification
**Microstep:** a Pass-3 golden-curl-stage **fix** to the deployed B8d `theo_message` (Plan Tier B8, "gateway integration"). The attachment-injection path reads blobs via the Function's managed identity, but the gateway's `requestUrl` cannot reach the MI token endpoint (it is `http://`, and `requestUrl` is HTTPS-only). No contract/behaviour change beyond making attachment reads actually work; chat, memory, history-RAG, persistence, and the zero-attachment path are untouched.

## P2 — Architecture & boundary reconciliation
- **Single localized fix.** Add `const http = require("http")`; in `requestUrl` and `requestBinary`, select `const lib = url.protocol === "http:" ? http : https` and call `lib.request(...)`. Everything else in the deployed B8d handler is unchanged.
- **Why it works:** all of `theo_message`'s outbound calls except the MI token are HTTPS (Foundry, AAD token, embeddings, Search, blob) — those continue to use `https` exactly as before; only the `http://` MI `IDENTITY_ENDPOINT` now resolves to the `http` lib. This is byte-for-byte the protocol selection the deployed B8b/B8c handlers already use (and which makes their MI blob reads succeed).
- **Boundary.** No `reporting_*`; no schema/DDL; no dependency (`http` is a Node built-in); no env; `theo_finalize_attachment`/`theo_create_attachment_upload`/`theo_delete_attachment` untouched.

## P2.5 / GR — Gap Register
Grounded against Governor §8 (closed vocabulary `PROCEED`/`PRE-LAND`/`ESCALATE`/`NO-GAPS`).
| Gap | Disclosure | Pivot |
| --- | --- | --- |
| G-1 | **Redeploy (Walter).** Replace **only** `theo_message/index.js` with §H (route + function.json unchanged). No migration/dependency/env. | **PRE-LAND** — §DEPLOY; Claude Code re-runs the §GOLDEN payoff round-trip after. |
| G-2 | **Infra already in place.** MI Storage Blob Data Contributor live (B8b/B8c). This fix only changes how the MI **token endpoint** is reached (http vs https). | **PROCEED** — no infra change. |
| G-3 | **API-Spec §2.8 read-path status** advances to "gateway injection DEPLOYED" once green. | **PRE-LAND** — short Role-C after green curls (unchanged from B8d). |

No write SQL. No `reporting_*` change.

## P3 — Backend / contract grounding
No contract change. Identical request/response. The corrected handler simply succeeds at the MI token fetch, so `buildAttachmentBlocks` produces real document/image/text blocks instead of "could not be loaded" notes.

## P4 — Schema definition
None — no DDL.

## P5 — Component reference grounding
- **Artifact under fix:** deployed B8d `theo_message.index.js` (blob `da15fe3a`) — §H inlines the corrected file full verbatim.
- **EXACT-mirror precedent:** the deployed B8c `theo_finalize_attachment` `requestUrl` (blob `b829d98d`) uses `const lib = url.protocol === "http:" ? http : https` — this fix makes `theo_message`'s `requestUrl` identical in that respect (Golden §3 EXACT-mirror; no new-domain authorization).

## §CHANGESET — exact delta vs the deployed B8d `theo_message` (diff-verified, 3 lines)
1. **`const http = require("http");`** added beside `const https = require("https");`.
2. **`requestUrl`** — inserted `const lib = url.protocol === "http:" ? http : https;` and changed `https.request(` → `lib.request(`.
3. **`requestBinary`** — same protocol-aware change (defensive; blob GETs are HTTPS, so this is symmetry, not strictly required).
Nothing else differs from the approved/deployed B8d handler.

## P6 — Repository & active-surface grounding
Changed artifact: `theo_message/index.js` (this fix package; supersedes the B8d copy for deploy) + unchanged `function.json`. `node --check` clean; diff vs the deployed B8d handler is the 3-line §CHANGESET.

## P7 — Risk / regression
- **Tightly scoped:** the lib-selection touches only how each request picks `http` vs `https`; for every existing HTTPS URL the behaviour is identical (`url.protocol !== "http:"` → `https`).
- **Fixes the MI path:** the `http://` `IDENTITY_ENDPOINT` now uses `http` → the MI token succeeds → attachment blobs load.
- **No regression:** chat/memory/history-RAG/persistence and the zero-attachment path are unchanged; regression + negatives already passed pre-fix and are unaffected.
- **Empirical confirmation** is the post-redeploy payoff curl (the MI endpoint is internal to the Functions sandbox and cannot be exercised from the workstation); the root cause is a confirmed, visible code difference against the working B8c reference.

## P8 — VEP assembly
GCR (§3) + Rule Anchors (§5) open the pack; P1–P8 walked; Gap Register present (G-1/G-3 PRE-LAND; G-2 PROCEED); §FINDING documents the golden-curl evidence; §CHANGESET the 3-line diff; §H inlines the corrected handler full verbatim; §FJ the unchanged function.json; §GOLDEN the payoff re-run; §DEPLOY the redeploy-only step. Plan-only. On Codex APPROVAL, Walter replaces the `theo_message` handler; Claude Code re-runs the payoff round-trip (PDF + XLSX now expected to read), then the API-Spec read-path Role-C (G-3) and B8e.

---

## §FINDING — B8d golden-curl evidence (Pass-3; token via `az`, never printed)
```
(1) regression (no attachment)  theo_message "reply ok"            -> 200, text "ok"                         PASS
(4) negatives                   non-owned id 404; bad uuid 400; 11 ids 400                                   PASS
(2) native PDF payoff           upload PDF + attachment_ids         -> 200, but Claude: "the file ... failed
                                                                       to load — I'm unable to access it"    FAIL
(3) extract XLSX payoff         upload XLSX + attachment_ids        -> 200, "wasn't able to load the file"   FAIL
```
Both payoff answers are the `buildAttachmentBlocks` top-level catch note — `getManagedIdentityAccessToken` threw because `requestUrl` issued the `http://` MI `IDENTITY_ENDPOINT` call over `https`. The deployed B8c `requestUrl` is protocol-aware and its MI blob reads succeed; this fix makes `theo_message` match.

## §H — `theo_message/index.js` (complete; B8d MI-protocol fix; full verbatim)
```js
...
## §FJ — `theo_message/function.json` (unchanged)
```json
{
  "bindings": [
    {
...
## §GOLDEN — payoff re-run (Claude Code, post-redeploy; token via `az`, never printed)
1. **regression** — plain chat → 200, "ok" (unchanged).
2. **native PDF** — upload a PDF saying "Da Vinci Fund III LP — Form 1065 — net income $42,000"; `theo_message` with its `attachment_ids` + "entity + net income?" → **200**, reply contains **"42,000"** and **"Da Vinci"**.
3. **extract XLSX** — the SheetJS probe workbook; ask its Total → **200**, reply references the value.
4. **negatives** — non-owned id 404; bad uuid 400; 11 ids 400.
5. cleanup + linkage confirm. Evidence under `.local/`.

## §DEPLOY — Walter deploy step
1. Replace **only** the `theo_message` function `index.js` with §H (route + `function.json` unchanged; no migration/dependency/env).
2. Reply "B8d MI-fix redeployed" → Claude Code re-runs §GOLDEN (now expecting the PDF/XLSX payoff to read), captures evidence, then prepares the API-Spec read-path Role-C (G-3) and begins B8e.

**Requested Pass 2 verdict:** Codex APPROVED or REJECTED.

*End of B8d MI-Endpoint Protocol Fix Pass-1 Backend VEP (plan only).*
