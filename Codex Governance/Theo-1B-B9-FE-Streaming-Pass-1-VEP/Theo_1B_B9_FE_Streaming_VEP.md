...
| G-7 | **Sidecar stream base must be configured.** `theo_message_stream` is on a **separate app** (`vaultgpt-func-stream`), so `streamBase` must be set — `VITE_STREAM_FUNCTIONS_URL` in the Theo SWA build (mirroring how `VITE_FUNCTIONS_URL` sets the monolith base), or `configureGateway({ streamBaseUrl })` at the Origin mount. If unset, chat safely degrades to the non-streaming monolith path (no streaming, but no breakage); the monolith `apiBase` and all other endpoints are never affected. | **PRE-LAND** — §DEPLOY; set `VITE_STREAM_FUNCTIONS_URL` for streaming to actually occur on the dev SWA. |
...
| `src/theo/services/gateway.live.ts` | +`sendMessageStream(req, handlers)` + `StreamHandlers`/`StreamCitation`; **+ a distinct `streamBase`** (the sidecar host — from `VITE_STREAM_FUNCTIONS_URL` at build or `configureGateway({ streamBaseUrl })`), used **only** by `sendMessageStream` so the monolith `apiBase` is untouched. Fetches `${streamBase}/api/theo_message_stream`, reads the `response.body` reader, dispatches SSE deltas (text/thinking/citation) + `vault_meta`/`vault_error`. Degrades: no live backend → mock once; `streamBase` unset → non-streaming `sendMessage` on the monolith (whole reply once). | `POST ${streamBase}/api/theo_message_stream` (SSE) — the B9 sidecar contract |
| `src/theo/services/theoClient.ts` | +`sendMessageStream` passthrough; `configureGateway` opts += `streamBaseUrl?` | single service boundary (1A handover §2.3) |
...
The five changed files are written and **validated against the live repo toolchain this turn**: `npm run typecheck` (`tsc --noEmit -p tsconfig.app.json`) → **clean**; `eslint` on all five → **clean (exit 0)**; `npm run build` → **clean** (the `TheoSurface` federated bundle builds, 55→59 kB with the streaming additions). Staged in `Codex Governance/Theo-1B-B9-FE-Streaming-Pass-1-VEP/proposed-src/theo/` (5 files); NOT yet applied to `src/` (applied at Pass 3 on approval). **Re-validated after the Pass-2 routing fix** (distinct `streamBase` for the sidecar; monolith `apiBase` untouched) — typecheck + eslint + build all re-run clean. Region classification (Golden Component Pack §4): the existing bubble/composer = **VISUAL-AUTHORITY-MATCH** (unchanged); `StatusLine` + `ThinkingPanel` + the streaming render = **VISUAL-AUTHORITY-DEVIATION** (additive, §WALTER-AUTH); service/state/types = **ALLOWED DELTA** (service-boundary streaming sibling + contract types, 1A handover §2.3).

## §DEPLOY / Pass-3 path
1. Codex Pass 2 reviews this VEP + CCT. 2. On APPROVAL, Pass 3 applies `proposed-src/` to `src/`, re-runs `typecheck`+`eslint`+`build`, emits the Component Structural Mirror + SWA test plan. 3. **Set the sidecar stream base (G-7):** add `VITE_STREAM_FUNCTIONS_URL=https://vaultgpt-func-stream-cyb4g8bhatddencs.uksouth-01.azurewebsites.net` to the Theo SWA build configuration (alongside the existing `VITE_FUNCTIONS_URL` monolith base). 4. Walter merges to `development` + SWA-tests on the dev SWA: send a message → status word appears within ~1s, the answer streams in token-by-token, the turn persists (reload shows it); (optionally) set `THEO_THINKING_BUDGET_TOKENS` on the sidecar → the Thinking panel lights up. 5. Pass 3 Visual Acceptance Evidence (recording/screenshot). No backend deploy step (the sidecar is live).
