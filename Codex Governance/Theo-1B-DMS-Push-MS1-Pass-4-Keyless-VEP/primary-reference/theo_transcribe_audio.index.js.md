# Primary Reference (Golden Handler §2) — deployed `theo_transcribe_audio` `index.js`

Canonical Primary Reference for MS1 Pass-4. Deployed on `vaultgpt-func-chat` (HF-T6 audio gateway broker). Retrieved this turn from the live Kudu VFS (`/site/wwwroot/theo_transcribe_audio/index.js`, HTTP 200). Inlined FULL VERBATIM (no ellipsis). The Pass-4 `dms_subscribe` mirrors this handler's `requestUrl` (http/https-aware) and `getManagedIdentityAccessToken(resource)` helpers EXACT; the sole delta is the token audience (`resource`), Graph instead of Cognitive Services.
