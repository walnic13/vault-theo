...
| - | --- | --- | --- | --- | --- |
| TC-1 | `useTheoState.ts` (**ACTIVE**, modify — state owner) | Adds `const [queued, setQueued] = useState<string \| null>(null)` and `const queuedRef = useRef<string \| null>(null)`. `send()`'s guard becomes `if (uploading) return;` then `if (loading) { if (text) { setQueued(text); queuedRef.current = text; setDraft(""); } return; }` before the existing `if (!text && ready.length === 0) return;`. New flush effect: `useEffect(() => { if (loading \|\| !queuedRef.current) return; const q = queuedRef.current; queuedRef.current = null; setQueued(null); void send(q); }, [loading])`. In the real-error `catch` (after the existing rollback): `queuedRef.current = null; setQueued(null);`. **Rev 2 (Pass-2 fix):** in the abort branch of `send()`'s `catch`, the **implicit-abort** case (`!userStoppedRef.current` — a chat switch / newChat aborted the stream) ALSO clears the queue (`queuedRef.current = null; setQueued(null);`) so the flush effect (which fires on `loading` → false) cannot send the queued follow-up into the newly-loaded thread; the **user-Stop** case leaves the queue intact (it auto-sends into the same thread). New `function cancelQueued() { queuedRef.current = null; setQueued(null); }`. Return object gains `queued` + `cancelQueued`. | VA-T1 (state owner; no direct surface) | none new — reuses the deployed B9 stream via `send()` | PROCEED |
...
| **G-5** | **Cross-thread send on switch-abort — RESOLVED (Codex Pass-2 finding).** A chat switch (newChat/selectRecent/startInProject) aborts the in-flight stream, which sets `loading` false; without clearing the queue, the flush effect would send the queued text into the new thread. **Fix:** the implicit-abort branch of `send()`'s `catch` now discards the queue (`queuedRef.current = null; setQueued(null);`). This runs before the effect (catch → finally `setLoading(false)` → re-render → effect), so `queuedRef` is already null and no flush occurs. The user-Stop branch intentionally preserves the queue (same-thread follow-up). Centralized in the one place every in-flight switch funnels through (the abort → catch), covering `deleteConversation` and the host `newChatNonce` too. | **PROCEED (resolved)** — verified `tsc --noEmit` exit 0. |
