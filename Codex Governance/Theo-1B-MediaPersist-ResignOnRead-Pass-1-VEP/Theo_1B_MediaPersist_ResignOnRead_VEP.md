...
| 10 | **T12 authorized-helper source** — deployed premium `theo_create_attachment_upload` (MI/UDK/SAS helpers) — `.../primary-reference/theo_create_attachment_upload.DEPLOYED.index.js` | `Read(full)` this turn | `ed24354de518469f2cb2a061f5b3a7da2e4c7998` |
| 11 | **PROPOSED (deploy artifact)** — `theo_get_conversation/index.js` (baseline + re-sign) | authored + syntax-checked this turn | `6653a8ca9f4657c1c3a8d0fa0e0e5d870d7933a3` |
...
**T12 AUTHORIZED-HELPER SOURCE (Golden Handler §4 — EXACT mirror against a deployed handler containing the helper):** `primary-reference/theo_create_attachment_upload.DEPLOYED.index.js` — blob `ed24354de518469f2cb2a061f5b3a7da2e4c7998` (deployed premium). The MI/UDK/SAS helpers — `requestUrl`, `getManagedIdentityAccessToken`, `xmlEscape`, `encodeBlobPath`, `decodeXmlTag`, `toIsoNoMillis`, `getUserDelegationKey`, `computeUserDelegationSignature` — are **spliced byte-verbatim** into the proposed handler (verified this turn: the donor's lines 95–220 appear verbatim in the proposed `index.js`). Premium's MI already holds the storage access these use (it mints attachment SAS against the same account/container), so **no new grant**.
...
| MI/UDK/SAS helper block (`requestUrl` … `computeUserDelegationSignature`) | deployed `theo_create_attachment_upload` lines 95–220 | **EXACT** (byte-verbatim splice; T12 authorized-helper reuse) |
...
3. **GC-3 (re-sign heals):** authenticated `GET …?conversationId=<the persisted image conversation>` → **200**; assert each `data.messages[*].media.image.images[*].imageUrl` that targets `vaultgptstorage01.blob.core.windows.net/theo-content/images/<oid>/…` carries a **fresh** `se=` (expiry ≈ now + 60 min) and a `sig=`; then a follow `HEAD`/`GET` of that re-signed URL → **200** (blob served) — proving the token resolves post-reload. *(Walter's hard-refresh of the Chipper Jones chat is the runtime Visual Acceptance.)*
...
## P8 — VEP assembly
GCR + Rule Anchors open the pack; P1→P7 walked; Gap Disclosure (G-1…G-4 PROCEED); Primary Reference = deployed `theo_get_conversation` `index.js`+`function.json` (committed full-verbatim, byte-identical to live); T12 helper reuse from deployed `theo_create_attachment_upload` (byte-verbatim splice verified). Change is additive (one baseline line changed, `diff`-verified); `node -c` passes. No contract/schema/migration/FE/`function.json` change. On Codex APPROVAL, Pass 3 deploys `index.js` to premium via DR-T15 Kudu VFS + runs GC-1…GC-4; Walter's hard-refresh of the persisted image chat (images return) = runtime Visual Acceptance.
