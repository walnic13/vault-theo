...
Pass-4 Role-C documentation amendment — the **schema-authority half of Part 1** (the DDL is `media_addendum.sql` in this same package). It documents the additive `theo_messages.media jsonb NULL` column in `spec/THEO_AZURE_POSTGRES_SCHEMA.md` §5 — the **deployed-schema truth owner** — so the schema authority lists `media` for the Part 2 (persist) + Part 3 (read) handler VEPs to ground against (Codex flagged the drift: the schema spec still stopped at `citations`). Applied on the migration landing: Claude Code applies these edits only **after** Walter has executed `media_addendum.sql` (verified live via `media_verify.sql`), so the schema doc always reflects the deployed column. Mirrors the B8c/B8i addendum-documentation pattern. Two additive edits (the `theo_messages` §5 row + the additive-columns list). No code change; the migration itself is Walter-executed (`media_addendum.sql`).
...
## Requested action
Codex Pass-4 Role-C review (APPROVED / REJECTED only). On APPROVED, Claude Code applies Edits 1–2 byte-faithfully to `spec/THEO_AZURE_POSTGRES_SCHEMA.md` **after Walter has executed `media_addendum.sql`** (confirmed live via the read-only `media_verify.sql`; `THEO_AZURE_POSTGRES_SCHEMA.md` is the deployed-schema truth owner, so it documents the column only once deployed) and commits — after which the Part 2 + Part 3 handler VEPs + the §2.1 Role-C re-ground against the updated schema blob and resubmit.
