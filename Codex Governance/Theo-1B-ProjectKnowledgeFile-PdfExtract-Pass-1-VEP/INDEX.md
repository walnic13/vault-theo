...
| c:/Users/WalterMansfield/Vault Group LLP/Innovate - Documents/Tax Workpapers Project/2026/vault-theo/spec/THEO_API_SPEC.md | §2.2 | "add project knowledge from a file" | §1 — same route, response shape unchanged (no contract change) |
...
- **Boundary:** one handler edit + `pdf-parse@1.1.1` added to the app package. No schema/contract change (response shape identical); no premium change; no FE change (the FE already calls this route). `node --check` PASS.
...
- **(2) No schema/contract change (P3/P4).** Same `theo_project_knowledge` columns; same `POST /api/theo_add_project_knowledge_file` response shape (API Spec §2.2, already deployed/documented). PROCEED.
...
3. No Role-C (route/response unchanged; §2.2 already documents the endpoint).
