...
Turn issued against HEAD: `2a33fdb0ede91f924aaa4f0b1cf9c47a57733697` (vault-theo, `development`; grounding parent `7e0c0b8cf1a82b536173cb59b29327e0696ba5cb`). Working tree also carried untracked `artifacts/*.xlsx` (Class B disclosed workbook dirt — not source/governance; not used as grounding evidence this turn).
...
- **Boundary:** one handler edit + `pdf-parse@1.1.1` added to the app package. No schema change; response *shape* identical; no premium change; no FE change (the FE already calls this route). **Contract behavior change** (native text PDF `400`→`201`) is documented via a **PRE-LAND §2.2 Role-C** (Gap Register §2). `node --check` PASS.

## §2 Gap Register
**PROCEED**, with one **PRE-LAND** (the §2.2 behavior Role-C, item 2).
...
- **(2) Contract BEHAVIOR change — PRE-LAND §2.2 Role-C.** No schema change and the response *shape* is identical, but the *documented behavior* changes: API Spec §2.2 currently states native PDFs ≤ 3 MB → 400 `UNSUPPORTED_MEDIA_TYPE`, and this microstep makes native *text* PDFs → 201. **PRE-LAND:** a §2.2 Role-C corrects that note (text PDFs of any size accepted — extracted via `pdf-parse` when native; only images / scanned-image PDFs with no text layer → 400), authored + landed in Pass-4 sequenced with this deploy (immediately after the golden curls confirm the native-PDF 201). This replaces the earlier "No Role-C" claim. PRE-LAND.
...
- **P4 Contract grounding:** no change — same route + response shape (API Spec §2.2).
...
- [x] No schema/contract change (same columns; same response shape).
...
3. **Land the PRE-LAND §2.2 behavior Role-C (Pass-4):** correct the API Spec §2.2 file-knowledge row's native-PDF note (native text PDFs → 201 via pdf-parse; only images / scanned-image PDFs → 400), immediately after the golden curls confirm the native-PDF 201 — so the doc matches the deployed behavior (no T13 drift).
