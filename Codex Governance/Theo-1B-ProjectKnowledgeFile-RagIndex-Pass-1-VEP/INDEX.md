...
Turn issued against HEAD: `ea49d642dca8fb08230d190714578ea391f7a9ba` (vault-theo, `development` — the commit that contains this complete package body: the Primary Reference index.js and the reuse-source theo_index_messages helper bodies both inlined as **actual file bytes** (spliced from disk, verified byte-identical by diff — no reconstruction), the paired function.json inline, and the corrected structural-mirror classifications; this stamp is applied in the immediately-following child commit under review, since a commit cannot record its own SHA). Every §4 currency anchor below resolves identically at `ea49d642` and at the child commit. Working tree also carried untracked `artifacts/*.xlsx` (Class B disclosed workbook dirt — not source/governance).
