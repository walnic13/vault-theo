...
Turn issued against HEAD: `0f7237551f2a6a425e0b74e113978a6f4f920f76` (vault-theo, `development` — the commit that contains this complete package body: the Primary Reference index.js and the reuse-source theo_index_messages helper bodies both inlined as **actual file bytes** (spliced from disk, verified byte-identical by diff — no reconstruction), the paired function.json inline, the corrected structural-mirror classifications, and the packaged handler comments aligned to the mirror (handler blob edbb107f); this stamp is applied in the immediately-following child commit under review, since a commit cannot record its own SHA). Every §4 currency anchor below resolves identically at `0f723755` and at the child commit. Working tree also carried untracked `artifacts/*.xlsx` (Class B disclosed workbook dirt — not source/governance).
...
This is the exact, byte-faithful content of the deployed Primary Reference file (`Theo-1B-ProjectKnowledgeFile-PdfExtract-Pass-1-VEP/theo_add_project_knowledge_file/index.js`, blob `630ff773165f5dc30caa2cebed3751f4275f504e`), inlined in full this turn — no compaction, no reformatting:
```javascript
...
These are the **exact, byte-faithful deployed source bodies** from `theo_index_messages.index.js` (blob `665bdb36fe5e59dbe75dec4a88bc29c4c1519003`), inlined in full this turn — no compaction. In the D1 package handler: `getAadToken` + `embedBatch` are reused **byte-identically** (verified by diff against this source); `upsertDocs` is reused with the single change `SEARCH_INDEX` → `PK_SEARCH_INDEX` in the docs/index URL; `ensureIndex` reuses the same create-or-update PUT logic with the project field set (`knowledge_id`/`project_id`/`title`/`chunk_index` in place of `conversation_id`/`role`, and `PK_SEARCH_INDEX`). Those adapted forms are in the D1 package handler and classified in the Structural Mirror. All under the composite authorization.
```javascript
