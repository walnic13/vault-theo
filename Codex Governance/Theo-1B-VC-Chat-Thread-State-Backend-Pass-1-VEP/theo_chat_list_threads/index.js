...
module.exports = async function (context, req) {
  ...
  try {
    ...
    // VC (thread-state): also the caller's per-thread hidden/muted flags. hidden threads are excluded from
    // the list UNLESS they have unread (so a new message resurfaces a hidden DM — WhatsApp-archive semantics);
    // muted is returned so the FE can suppress the thread's badge/notifications.
    const rows = await client.query(
      ...
      SELECT
        ...
        COALESCE(mem.hidden, false) AS hidden,
        COALESCE(mem.muted, false) AS muted
      ...
      WHERE $1 = ANY (t.member_oids)
        ...
        AND NOT (COALESCE(mem.hidden, false) AND COALESCE(unread.cnt, 0) = 0)
    ...
    const threads = rows.rows.map((r) => ({
      ...
      // VC (thread-state): the caller's per-thread flags. hidden = removed from the Orbit list (resurfaces on
      // unread — see the WHERE); muted = the caller silenced this thread's notifications/badge. Default false.
