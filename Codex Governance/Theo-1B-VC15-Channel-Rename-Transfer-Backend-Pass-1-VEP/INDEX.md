...
## R2 (Codex Pass 2, 2nd): whole admin-surface hardening
`transfer_admin` also raced the deployed `add_member`/`remove_member`. VC-15 now ALSO **modifies** those two (FOR UPDATE gate + `AND admin_oid=$caller` in the UPDATE + 0-row→403; remove also `AND $target<>admin_oid`). All four admin-mutating handlers serialize on the thread row and re-assert admin at write time.

## Contents
| File | Role |
| ---- | ---- |
| `Theo_1B_VC15_Channel_Rename_Transfer_Backend_VEP.md` | Pass 1 backend VEP (lint PASS): GCR + Rule Anchor Table + P1–P8 + Gap Register + §MIGRATION=none + Primary Reference (`theo_chat_add_member` pair, verbatim) + §HG (rename + transfer NEW; add + remove MODIFIED — R2) + §API-SPEC Role-C + §DEPLOY (2 new + 2 overwrite) + §CURL. |
| `theo_chat_rename_channel.index.js` / `.function.json` | NEW — admin rename (FOR UPDATE + admin-in-UPDATE). |
| `theo_chat_transfer_admin.index.js` / `.function.json` | NEW — admin transfer (new admin must be a different current member; FOR UPDATE + admin-in-UPDATE). |
| `theo_chat_add_member.index.js` / `.function.json` | MODIFIED (R2) — execution-time admin guard retrofit. |
| `theo_chat_remove_member.index.js` / `.function.json` | MODIFIED (R2) — execution-time admin guard + `$target<>admin_oid`. |
