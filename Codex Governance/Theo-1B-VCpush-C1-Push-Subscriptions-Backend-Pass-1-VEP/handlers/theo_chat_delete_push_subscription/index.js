...
module.exports = async function (context, req) {
  ...
  // Validate BEFORE any SQL (deterministic 400).
  ...
  try {
    ...
    // Idempotent, owner-scoped delete: the ownership DELETE RLS (created_by = auth.uid()) plus the explicit
    // created_by = auth.uid() predicate guarantee a caller can only remove their OWN subscription. A missing
    // row (already unsubscribed) removes 0 rows and is still reported as success (idempotent).
    const del = await client.query(
      `DELETE FROM public.theo_chat_push_subscriptions WHERE endpoint = $1 AND created_by = auth.uid()`,
      [endpoint]
    );

    return send(context, 200, successBody({ deleted: del.rowCount > 0, endpoint }));
  } catch (err) {
