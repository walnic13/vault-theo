...
| c:/Users/WalterMansfield/Vault Group LLP/Innovate - Documents/Tax Workpapers Project/2026/vault-theo/governance/THEO_GOLDEN_HANDLER_STANDARD.md | §3 | "rejects unknown/extra fields" | §4 — `ALLOWED_BODY_KEYS = {conversation_id, starred}` allowed-key check → 400 on any unknown/extra body field |
...
| `theo_set_conversation_starred` `module.exports` | owner-scoped set-field mirroring the sibling: OID gate (401), unknown/extra-field rejection (400), `conversation_id` uuid + `starred` boolean validation (400), `SET starred` UPDATE, exists-unscoped 403/404; **drops** the project-access check + set-once guard + the `updated_at` bump; **adds** an `ALLOWED_BODY_KEYS` unknown-field rejection (Golden Handler §3) that the deployed sibling lacks | **ALLOWED DELTA** (the star toggle + the §3 unknown-field conformance addition) | Golden Handler §3/§4 |
...
1. **Unauth** → `401`. **`{}`** → `400`. **Unknown/extra field** `{conversation_id:<uuid>, starred:true, extra:true}` → `400 INVALID_REQUEST`. **Non-boolean starred** `{conversation_id:<uuid>, starred:"yes"}` → `400`. **Bad uuid** → `400`.
