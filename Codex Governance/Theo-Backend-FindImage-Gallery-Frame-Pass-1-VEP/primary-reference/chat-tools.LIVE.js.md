# Primary Reference — deployed `engine/chat-tools.js` on vaultgpt-func-stream (LIVE)
...
const CHAT_TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      'The app displays the returned image to the user AUTOMATICALLY (like a download card) — you do NOT ' +
      'need to and MUST NOT emit a markdown image or paste the URL yourself; just briefly confirm and/or ' +
      'describe what is shown. When source is "openverse", mention the creator and license in your text ' +
      '(e.g. "Photo: <creator> — <license>"). If it returns an error or no image, tell the user you could ' +
      'not find one rather than inventing a URL.',
    input_schema: {
      type: 'object',
      properties: {
