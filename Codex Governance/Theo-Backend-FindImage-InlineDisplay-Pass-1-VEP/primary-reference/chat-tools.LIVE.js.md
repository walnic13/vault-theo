# Primary Reference — deployed `engine/chat-tools.js` on vaultgpt-func-stream (LIVE, post-FindImage-registration)

Kudu VFS GET `/site/wwwroot/src/engine/chat-tools.js`, 2026-07-23 (includes the theo_find_image entry from the registration VEP). Verbatim, full.
...
const CHAT_TOOLS = [
  ...
  },
  {
    ...
    downloadable: false,
    description:
      'Find a REAL, directly-viewable image URL for a subject so you can SHOW the user what it looks ' +
      'like. Input: { subject }. Pass a CONCISE subject noun (e.g. "golden retriever", "Eiffel Tower", ' +
      '"latte art") — not a whole sentence. Returns { source, title, imageUrl, fullImageUrl, pageUrl, ' +
      'license, creator }. Use this whenever the user asks to see / show / picture what something looks ' +
      'like, INSTEAD of guessing an image URL yourself — guessed image URLs usually 404 (broken image). ' +
      'After it returns, display the image by emitting a markdown image exactly as ![title](imageUrl), ' +
      'using imageUrl verbatim (do not alter it). When source is "openverse", also add a short caption ' +
      'crediting the creator and license (e.g. "Photo: <creator> — <license>"). If it returns an error ' +
      'or no image, tell the user you could not find one rather than inventing a URL.',
    input_schema: {
      type: 'object',
      properties: {
        subject: { type: 'string', description: 'Concise subject to find an image of, e.g. "golden retriever" or "Eiffel Tower".' },
      },
      required: ['subject'],
    },
  },
