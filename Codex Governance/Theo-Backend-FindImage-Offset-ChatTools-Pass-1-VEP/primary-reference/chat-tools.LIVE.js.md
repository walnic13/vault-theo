# Primary Reference — DEPLOYED `chat-tools.js` (LIVE) — vaultgpt-func-stream `/site/wwwroot/src/engine/chat-tools.js`

GET-verified from Kudu this turn. Verbatim, full — no ellipsis. This VEP adds only the `offset` guidance to the `theo_find_image` description + an `offset` `input_schema` property; everything else byte-identical.
