...
## §2 Gap Register
**PROCEED.**
- **(G-1) `jimp` dependency — PINNED `jimp@0.22.12` (v0.x).** Pure-JS, no native binary → deploys via the Kudu node_modules path (like `pg`). Lazy-required → no cold-start impact on text turns. **PINNED** because jimp v1.x changed the API the helper uses (named import / `getBuffer` / moved `MIME_*` / resize signatures) — a bare install would pull v1.x and throw for every non-GIF image (defeating the fix). The helper targets the v0.x API; `0.22.12` matches it, **verified this turn** (installed 0.22.12, ran the exact `Jimp.read`/`MIME_*`/`scaleToFit`/`quality`/`getBufferAsync` calls — all OK). Deploy pins it (§6 step 2). This mirrors the established pin discipline (cf. pdf-parse @1.1.1). Disclosed, PROCEED.
...
2. **Add `jimp` — PINNED to `jimp@0.22.12`:** `npm install jimp@0.22.12 --omit=dev` locally (NOT a bare `npm install jimp`); deploy its `node_modules/jimp` (+ transitive deps) into `/site/wwwroot/node_modules/` via the Kudu **zip API** (`PUT /api/zip/site/wwwroot/node_modules/`, which merges — leaves existing modules intact), same technique as the TODO-store `pg` deploy. **The pin is load-bearing:** jimp **v1.x renamed the API** (named import `const { Jimp } = require("jimp")`, `getBufferAsync`→`getBuffer`, moved `MIME_*` constants, changed resize signatures), which the helper does NOT use — a bare/unpinned install would pull v1.x and every non-GIF image would throw → degrade to a note (the pdf-parse pin lesson). The helper is written to the **v0.x** API; `jimp@0.22.12` matches it — **verified this turn** by installing 0.22.12 and exercising the exact calls (`Jimp.read`, `Jimp.MIME_JPEG`/`MIME_PNG`, `scaleToFit(1568,1568)`, `quality(82)`, `getBufferAsync`) — all OK, a 3000×2000 image resized to a 1568 long edge.
