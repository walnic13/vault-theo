...
// Model per-image size guard (Claude/Foundry rejects images larger than ~5MB each; base64 adds ~33%). Images
// whose long edge exceeds IMAGE_MAX_EDGE, or whose bytes exceed IMAGE_RESIZE_THRESHOLD_BYTES (~3.75MB), are
// downscaled + re-encoded before injection so a large phone photo no longer 400s the upstream. See
// prepareImageForModel. Defaults: long edge 1568px (Anthropic's recommended max), threshold 3,932,160 bytes.
const IMAGE_MAX_EDGE = parsePositiveInt(process.env.THEO_IMAGE_MAX_EDGE, 1568);
const IMAGE_RESIZE_THRESHOLD_BYTES = parsePositiveInt(process.env.THEO_IMAGE_RESIZE_THRESHOLD_BYTES, 3932160);
...
// the image is oversized (dimensions or bytes), so it fits the model's per-image size limit (Claude ~5MB).
// Small images pass through unchanged (media_type preserved). jimp is LAZY-REQUIRED (pure JS — no native
// binary; loaded only when an image is actually resized) so text-only turns never pay for it. GIFs are not
// re-encoded (would break animation) — injected as-is when within budget, else skipped. Returns
// { data(base64), mediaType } or null on failure (the caller degrades to a note; never throws the turn).
async function prepareImageForModel(buf, contentType, context) {
  try {
    ...
    // Preserve PNG (alpha) when the resized PNG is small enough; otherwise re-encode JPEG (photos) to
    // guarantee the size drop below the model limit.
    ...
    context.error("theo_message_stream: image prepare/resize failed (non-fatal)", e && e.message);
    return null;
  }
}
  ...
  for (const row of rows) {
    ...
    try {
      if (native) {
        ...
        nativeBytes += buf.length; // B8k: no cap on ORIGINAL bytes (Walter-directed). Documents/extracted text
        // inject in full as before; only IMAGES are downscaled below, and only to fit the model's per-image limit.
        if (native === "document") {
          blocks.push({ type: "document", source: { type: "base64", media_type: row.content_type, data: buf.toString("base64") } });
          blocks.push({ type: "text", text: `(above is the attached file "${row.filename}")` });
        } else {
          // Downscale large images so they fit the model's per-image size limit (Claude ~5MB). A full-size
          // phone photo (7-8MB) otherwise 400s the upstream → "couldn't reach the assistant". Non-oversized
          // images pass through unchanged; a resize failure degrades to a note (never throws the turn).
          const prepared = await prepareImageForModel(buf, row.content_type, context);
          if (prepared) {
            blocks.push({ type: "image", source: { type: "base64", media_type: prepared.mediaType, data: prepared.data } });
            blocks.push({ type: "text", text: `(above is the attached file "${row.filename}")` });
          } else {
            blocks.push({ type: "text", text: `[Attached image "${row.filename}" could not be prepared for the model (too large or unreadable) — ask the user to resize/compress it.]` });
          }
        }
