# Primary Reference — DEPLOYED `theo_message_stream.js` (LIVE) — vaultgpt-func-stream `/site/wwwroot/src/functions/theo_message_stream.js`

GET-verified from Kudu this turn. Verbatim, full — no ellipsis. This VEP bumps only the two server-tool `type` version strings (`web_search_20250305`→`_20260209`, `web_fetch_20250910`→`_20260209`) + a comment; everything else is byte-identical.
