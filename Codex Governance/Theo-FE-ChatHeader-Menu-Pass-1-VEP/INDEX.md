...
Supporting (non-component) edits: `types.ts` (`ConversationSummary.starred?: boolean` — **optional**: `theo_list_conversations` returns it but `theo_get_conversation` does not, and `ConversationDetail.conversation` reuses `ConversationSummary`, so a required field would overstate the get/reload read-contract; the menu sources `starred` from the list, §2(7)), `services/theoClient.ts` + `services/gateway.live.ts` + `services/gateway.mock.ts` (`setConversationStarred` through the gateway → `POST /api/theo_set_conversation_starred`, mock no-op), `components/icons.tsx` (`IcStar` w/ `filled`, `IcChevron` — VA-T1 SV idiom).
...
- **(7) Read-contract soundness (T22/T13) — `starred` is OPTIONAL on `ConversationSummary`.** `theo_list_conversations` returns `starred` (deployed + documented), but `theo_get_conversation` does **not**, and `ConversationDetail.conversation` reuses `ConversationSummary`. Typing `starred` as required would overstate the get/reload read-contract (the detail object legitimately omits it). So `starred?: boolean` is optional — sound for both the list (present) and the detail (absent). The chat-header menu never reads `starred` off a detail object: `currentConversation = recentsList.find(...)` (the LIST) feeds `ChatMenu`, and `ChatMenu` reads `!!conversation.starred` (undefined → unstarred). No `theo_get_conversation` change needed. Resolves the prior REJECT. PROCEED.
