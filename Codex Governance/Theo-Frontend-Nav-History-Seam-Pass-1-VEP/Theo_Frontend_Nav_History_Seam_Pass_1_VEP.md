...
| G-PUSH-DEDUP | Not every nav call is "forward" (re-clicking the current view; the cold-open restore of the last chat); and `newChat()` today routes through `go("chats")`, which would double-push (once for the leaving chat, once for the nested `go`). | **PROCEED** | The state transition is separated from the history push: a **non-pushing** primitive `applyView(v)` (the current `go` body minus the push) performs the view change; `pushNav()` is called **exactly once** at each public forward-nav entry, and no forward-nav routes through a *pushing* call. Specifically `newChat()` is reworked to call `applyView("chats")` (NOT `go("chats")`), so the whole newChat transition is single-owned for history (one leaving-location push, no spurious fresh-chat entry). `pushNav` still records the LEAVING location only when it differs from the stack top, and the cold-open restore path is guarded (`navRestoringRef`) so it seeds no bogus entry. This closes the Codex T13. |
...
```
**Push discipline (Codex T13 close):** the history push is separated from the state transition. `applyView(v)` performs the view change and NEVER pushes. `go(v)` = `pushNav(); applyView(v)` — the public forward nav. `newChat()` is reworked to call `applyView("chats")` (NOT the pushing `go("chats")`), so its transition pushes exactly once (the leaving location) with no spurious fresh-chat entry. `openProject` / `selectRecent` / `startInProject` each call `pushNav()` once at entry and already set `view`/`detailId` directly (they do not route through `go`), so no nested push. Net invariant: **exactly one `pushNav()` per user-initiated forward navigation; no forward nav routes through another pushing call.** `goBack()` pops the top, sets `navRestoringRef`, re-invokes the matching nav fn for that `NavLoc` (`go(view)` / `selectRecent(id)` / `openProject(id)` / `newChat()`) — all pushes suppressed by the ref — then clears it. `navDepth`/`navContextTitle` are derived from `navStack` + the current loc + `chatProject`/`currentConversation`.
