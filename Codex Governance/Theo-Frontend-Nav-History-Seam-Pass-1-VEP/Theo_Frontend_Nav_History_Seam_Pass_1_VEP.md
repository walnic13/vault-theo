...
| G-PUSH-DEDUP | Not every nav call is "forward": re-clicking the destination you are already on must push nothing; the cold-open restore of the last chat must seed nothing; and `newChat()` today routes through `go("chats")` (which would double-push). | **PROCEED** | **Destination-aware dedupe** (not stack-top compare): each public nav computes its resulting `NavLoc` and calls `pushNavIfDestinationChanges(target)`, which pushes the leaving `currentLoc()` **only when `currentLoc()` != `target`** — so tapping New chat while already on a fresh chat (or re-selecting the active view) pushes nothing, even when the stack top is an older location. The state transition is separated from the push via a non-pushing `applyView(v)`; `newChat()` calls `applyView("chats")` (NOT the pushing `go("chats")`) so it is single-owned. The cold-open restore path is guarded (`navRestoringRef`) so it seeds no entry. Net: at most one push per forward nav, none on a no-op nav. This closes both Codex T13 findings. |
...
function currentLoc(): NavLoc { /* derive from view/detailId/conversationId: project→{project,detailId}; projects/artifacts/customize→{view}; chats+conversationId→{chat,id}; chats+no conversationId→{newchat} */ }
function locEquals(a: NavLoc, b: NavLoc): boolean { /* k-equal AND (view-equal for "view" | id-equal for "chat"/"project" | always for "newchat") */ }
function pushNavIfDestinationChanges(target: NavLoc) { /* if !navRestoringRef.current && !locEquals(currentLoc(), target): push currentLoc() */ }
function applyView(v: View) { /* NON-pushing view transition = today's go() body: setView(v); setDetailId(null); if (v==="artifacts") loadGalleryArtifacts(); */ }
```
**Push discipline (Codex T13 close — DESTINATION-aware dedupe):** the history push is separated from the state transition, and dedupe compares the CURRENT location to the nav's DESTINATION (not to the stack top). Each public forward-nav computes its resulting `NavLoc` and calls `pushNavIfDestinationChanges(target)`, which pushes the LEAVING location (`currentLoc()`) **only when you are not already at `target`** — so re-clicking the destination you are already on (e.g. tapping New chat while already on a fresh chat, or re-selecting the current view) pushes nothing and creates no dead Back step. Targets:
- `go(v)`: `v === "chats"` → `conversationId ? {k:"chat",id:conversationId} : {k:"newchat"}` (go preserves the open conversation); else `{k:"view", view:v}`.
- `newChat()`: `{k:"newchat"}` — and it calls `applyView("chats")` (NOT the pushing `go("chats")`) for the transition, so the whole newChat is single-owned for history.
- `openProject(id)`: `{k:"project", id}`. · `selectRecent(id)`: `{k:"chat", id}`. · `startInProject(id)`: `{k:"newchat"}`.

`applyView(v)` NEVER pushes; `go(v)` = `pushNavIfDestinationChanges(target); applyView(v)`; `openProject`/`selectRecent`/`startInProject` set `view`/`detailId` directly (they do not route through `go`) and call `pushNavIfDestinationChanges(target)` once at entry. Net invariant: **at most one push per user-initiated forward navigation, and none when the nav is a no-op (already at the destination); no forward nav routes through another pushing call.** `goBack()` pops the top, sets `navRestoringRef`, re-invokes the matching nav fn for that `NavLoc` (`go(view)` / `selectRecent(id)` / `openProject(id)` / `newChat()`) — all pushes suppressed by the ref — then clears it. `navDepth`/`navContextTitle` are derived from `navStack` + the current loc + `chatProject`/`currentConversation`.
