...
v1 REJECTED (T22 / F-P2.5): routing on `app_key === 'sigma'` **alone** would misroute — the deployed Sigma mount already passes `appContext={{ app_key: 'sigma', app_context: null }}`, so today's general Sigma-dock turns (no review open) would hit `sigma_review_agent_stream` with `review_id`/`files` **missing** (contract violation), and the v1 gap note wrongly claimed "no `app_key==='sigma'` context arrives." Fix: a **fail-closed routing guard** — `hasReviewContext(appContext)` = `app_key === 'sigma'` AND `app_context.sigma_review_id` is a uuid AND `app_context.files.input`/`files.output` are present. Route to `sendReviewAgentStream` **only** when `hasReviewContext` is true; in **every** other case (including the current `app_context: null`) fall back to `sendMessageStream` (general Theo). So no current turn is misrouted, and the review agent activates only once a review is open (the paired Sigma-FE VEP supplies the payload). Updated F-P1, F-P2.5, F-P7, and the `useTheoState` CCT row.
...
**PROCEED.** (1) **`app_context` contract + fail-closed routing:** the deployed Sigma mount already passes `appContext={{ app_key: 'sigma', app_context: null }}`, so `app_key` alone is NOT a safe route predicate (it would send today's general Sigma-dock turns to `sigma_review_agent_stream` with `review_id`/`files` missing). The FE therefore routes on a **complete-payload guard** — `hasReviewContext(appContext)` = `app_key==='sigma'` AND `app_context.sigma_review_id` is a uuid AND `app_context.files.input`/`files.output` present. Only then → `sendReviewAgentStream`; in every other case (including the current `app_context: null`) → `sendMessageStream` (general Theo, fail-closed). The complete payload is supplied by the paired **Sigma-FE** VEP (Sigma passes `{ sigma_review_id, files }` when a review is open); until it lands the guard is false and the general chat path is used. Coupled, disclosed. PROCEED. (2) **VA-T7** registered this turn (`7fae8dd`) — cited, not pre-cited. PROCEED. (3) **Single service module:** the new call goes in `gateway.live.ts` (Governor §6), not inline. PROCEED. No PRE-LAND/ESCALATE.
...
| `useTheoState` (ACTIVE, modify) | (existing signature) — when `hasReviewContext(appContext)` is true (`app_key==='sigma'` AND `app_context.sigma_review_id` uuid AND `files.input`/`files.output` present), call `sendReviewAgentStream`; **else `sendMessageStream`** (fail-closed — covers the current `app_context: null`); hold `reasoning` + `tools[]` on the streaming assistant message via `patchLastAssistant` | VA-T1 (state) | the gateway (`sendReviewAgentStream`) | PROCEED |
...
3. **`useTheoState`**: define `hasReviewContext(appContext)` = `appContext?.app_key === 'sigma' && isUuid(appContext.app_context?.sigma_review_id) && appContext.app_context?.files?.input && appContext.app_context?.files?.output`; when true call `sendReviewAgentStream`, **else `sendMessageStream`** (fail-closed — the current `app_context: null` and all non-sigma turns keep the general path); add `onTool`/`onToolResult` that push/patch `tools[]` and hold streamed `reasoning` on the assistant message (via `patchLastAssistant`).
