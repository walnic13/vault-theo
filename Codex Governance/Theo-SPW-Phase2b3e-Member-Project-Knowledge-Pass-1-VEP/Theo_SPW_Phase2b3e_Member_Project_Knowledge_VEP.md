# Theo — Shared Project Workspace Phase 2b-3e (G-4): member project-knowledge RAG parity (func-stream) — Pass 1 Backend VEP

> Pipeline: Vault Theo backend regime. Author = Claude Code (Pass 1). Reviewer = **Codex** (Pass 2). Plan-only; on APPROVAL Claude Code deploys the one modified handler to `vaultgpt-func-stream` via **Kudu VFS** (PUT + GET-back byte-diff + restart) and runs authenticated golden curls. **Shared Project Workspace, Phase 2b-3e / G-4 (member/owner project-RAG parity).** Today `theo_message_stream`'s project-knowledge RAG injection is owner-scoped: the active-project lookup + the Azure Search filter + the live-filter all carry `created_by = the caller`, so a MEMBER continuing a published thread resolves no active project and gets NONE of the project's indexed knowledge (only the owner's turns do). This microstep gives members the same project context, SAFELY: **(1)** broaden the active-project lookup to by-id (a member resolves the conversation's project); **(2)** ADD a `theo_project_effective_role` **access-gate** on the resolved project (creator ∪ owner ∪ member) — else inject nothing, so a caller passing an arbitrary `project_id` can never surface a project's knowledge they cannot see; **(3)** drop `created_by` from the Azure Search filter (search the whole project's indexed knowledge) and **(4)** from the live-filter — both now safe behind the access-gate. No DB/DDL change; no FE change. `theo_message` (premium, non-stream) has NO project-knowledge RAG, so it is untouched (G-4 is stream-only).
...
Grounding parent (source baseline): `ef2901e7b9b4fbea92c8943ffddc787376a82243` (vault-theo, `development`) — this package is carried at a later reviewed commit named only in the forward submission note; all currency anchors below are tip-independent blob SHAs. The **Primary Reference is the LIVE deployed handler** pulled from func-stream Kudu VFS this turn (blob `4e71da84df081fd22c4efb302363bf0dcf077ff4` — the 2b-3c-ii-deployed version).
Grounding Mode: Full Baseline Grounding
Pass: Pass 1
Sub-phase Track: P8
Detail: Pass 1 backend VEP; P1–P8 walked. Single-handler read-broadening microstep — NO DB/DDL change (the reused `theo_project_effective_role` gate is the deployed Phase-1 role gate, schema §10). The modified `theo_message_stream` preserves the entire deployed scaffolding (v4 stream handler, EasyAuth OID, model/tool streaming, history-RAG, persistTurn, `set_config` RLS context) and changes ONLY the project-knowledge RAG block: by-id active-project resolution + a `theo_project_effective_role` access-gate + project-scoped (not caller-scoped) search/live filters (§P5 diff). The unrelated `searchHistory` (the caller's OWN cross-conversation recall) stays caller-scoped — untouched. Modified handler passes `node --check`. Deploy = Kudu VFS (Golden Handler §5.5); curl verification is Claude Code's job. Full Baseline per Conformance §4.
...
| 5 | Theo Azure Postgres Schema — `spec/THEO_AZURE_POSTGRES_SCHEMA.md` (§10 `theo_project_effective_role` role gate — the reused access classifier) | `Grep("theo_project_effective_role")` this turn | `abe14dc5d45b8a78b4d2b7303f0bd1257da120ec` |
| 6 | Theo API Spec — `spec/THEO_API_SPEC.md` (§2.2 project sharing; §2.1 chat) | carried grounding (this program; blob-anchored) | `fc0443eee2b598d8026cb40e073d4bc115a4a31e` |
| 7 | **Primary Reference (LIVE deployed handler)** — func-stream `theo_message_stream` (v4; pulled from Kudu VFS this turn) | `curl`(Kudu VFS GET) + `Read`(project-knowledge block + searchProjectKnowledge) + `node --check` this turn | `4e71da84df081fd22c4efb302363bf0dcf077ff4` |
| 8 | Deployed role-gate migration (canonical DDL, referenced) — `Codex Governance/Theo-SPW-Phase1-Roles-Substrate-Pass-1-VEP/spw_phase1_migration.sql` (`theo_project_effective_role`) | `Read` this program (Phase 1) | `75659097d611ba833741b2fb8383f7050c534334` |

No ChatGPT advisory cited (§4D / T18). No `reporting_*`/`corporate-reporting` change. No write SQL / no migration (the gate is already deployed). No FE change.
...
| spec/THEO_AZURE_POSTGRES_SCHEMA.md | §10 | "theo_project_effective_role" | §P2/§P5 — the access-gate reuses the deployed role classifier |

---

## P1 — Feature identification
SPW **Phase 2b-3e (G-4)**: make a project MEMBER's turn in a shared thread get the same project-knowledge RAG the OWNER's turns get — the last parity gap. Today the project-knowledge injection in `theo_message_stream` is caller-scoped at three points (active-project lookup, Azure Search filter, live-filter), so a member resolves no active project and gets no project knowledge. This microstep broadens those to project-scope, gated by a `theo_project_effective_role` access-check so it never leaks a project's knowledge to a non-participant. No new surface — the enabling backend parity change.

## P2 — Architecture & boundary reconciliation
- **Access-gate is the safety.** The pre-existing `created_by = caller` filters doubled as a leak guard: `body.project_id` is uuid-validated but NOT access-checked, so without the filters a caller could pass any project's id and pull its knowledge. This microstep replaces that implicit guard with an EXPLICIT one: after resolving `activeProjectId` (from `body.project_id` OR the conversation, by id), call the deployed `theo_project_effective_role(activeProjectId)` (schema §10 — Rule Anchor); if the caller is not a participant (creator ∪ owner ∪ member → non-null), `activeProjectId` is nulled and nothing is injected.
- **Broaden behind the gate.** With the gate in place: the active-project lookup is by-id (a member resolves the conversation's project); the Azure Search filter is `project_id eq …` only (the whole project's indexed knowledge, all authors); the live-filter is `WHERE project_id = $1` only. Each is safe because participation is confirmed first.
- **Non-fatal + unchanged elsewhere.** The block stays wrapped in the existing try/catch (a project-knowledge failure never breaks chat). `searchHistory` (the caller's OWN cross-conversation recall) is a different function and stays `created_by = caller` — untouched. Memory, attachments, persistTurn, the model/tool streaming — all unchanged.
...
| --- | --- | --- |
| G-1 | **Deploy (Claude Code, Kudu VFS).** PUT the modified handler to func-stream `/site/wwwroot/src/functions/theo_message_stream.js`; GET-back byte-diff; restart. v4 handler — no per-dir `function.json`. Rollback = re-PUT the retained LIVE baseline (blob `4e71da8`). | **PRE-LAND** — §DEPLOY; Claude runs §CURLS after. |
| G-2 | **Curls.** Owner streams a turn on an owned project-linked conversation → 200 SSE (project knowledge still injected — regression); a caller passing a `project_id` they do NOT participate in → 200 SSE with NO project knowledge injected (the access-gate nulls it; non-fatal). Member-gets-project-knowledge-200 needs a two-user fixture (verified via a manual two-account check — same limitation as 2b-3b/2b-3c). | **PRE-LAND** — §CURLS. |
| G-3 | **No API/schema/FE change.** The project-knowledge injection is an internal RAG-context enrichment (the response contract is unchanged); no §2.x edit needed. `theo_project_effective_role` is already deployed (schema §10). | **NO-GAPS** for docs. |

No write SQL. No `reporting_*` change.

## P3 — Backend / contract grounding
No contract shape change: `POST /api/theo_message_stream` (SSE) is unchanged. The change is internal — WHICH project's knowledge is retrieved into the model context for a member's turn (now the shared project's, gated by participation). No API-Spec edit needed. Errors/behavior otherwise unchanged.

## P4 — Schema definition
None. `theo_project_effective_role` (the reused access gate) + `theo_project_knowledge` are deployed (schema §10 / earlier tiers). This microstep consumes them.

## P5 — Component reference grounding (Primary Reference + handler delta)
**Primary Reference (Golden Handler §2 — exactly one deployed handler, inlined verbatim):** the **LIVE** `theo_message_stream` on func-stream (v4; blob `4e71da8`, the 2b-3c-ii-deployed version), pulled from Kudu VFS this turn. The **ALLOWED DELTA (§4 — the RLS-scoped query + the deployed governed classifier)** is exactly the unified diff below (by-id active-project lookup; the `theo_project_effective_role` access-gate; project-scoped search filter + call-site; project-scoped live-filter; + the block/function comments). Staged AFTER file: `handlers/theo_message_stream.js` (`node --check` clean). No per-dir `function.json` (v4 in-file `app.http` registration, unchanged).
...
+async function searchProjectKnowledge(searchToken, queryText, queryVector, projectId) {
+  // SPW G-4: filter by project only (all authors' project knowledge) — the caller's participation in
+  // the project is access-gated (theo_project_effective_role) before this runs, so the whole project's
+  // indexed knowledge is in scope, not just the caller's own uploads.
+  const filter = `project_id eq '${projectId.replace(/'/g, "''")}'`;
...
+    // Active project = explicit body.project_id (validated) else the conversation's project_id (by id —
+    // SPW G-4, so a project MEMBER continuing a shared thread resolves it too, not just the owner). The
+    // caller must PARTICIPATE in the resolved project (creator ∪ owner ∪ member — access-gated below via
+    // theo_project_effective_role) or NOTHING is injected; this prevents a caller passing an arbitrary
+    // project_id and surfacing a project's knowledge they cannot see. Search hits are intersected against
+    // live theo_project_knowledge rows (project-scoped — all authors' knowledge) so items
...
+            `SELECT project_id FROM public.theo_conversations WHERE id = $1`,
+            [requestedConversationId]
           );
           if (cp.rowCount > 0 && cp.rows[0].project_id) activeProjectId = cp.rows[0].project_id;
         }
+        // SPW G-4 access-gate: whether activeProjectId came from body.project_id (uuid-validated only)
+        // or the conversation, the caller MUST participate in it (theo_project_effective_role non-null:
+        // creator ∪ owner ∪ member) or we inject nothing — no cross-project knowledge leak.
+        if (activeProjectId) {
+          if (!pkClient) {
...
+            );
+          }
+          const accessRes = await pkClient.query(
+            `SELECT public.theo_project_effective_role($1::uuid) AS role`,
+            [activeProjectId]
+          );
+          if (!(accessRes.rows[0] && accessRes.rows[0].role)) activeProjectId = null;
+        }
...
+            searchToken, userText.slice(0, HISTORY_QUERY_MAX_CHARS), queryVector, activeProjectId
           );
           if (!pkClient) {
             pkClient = await pool.connect();
@@ -993,8 +1021,8 @@
...
-            [activeProjectId, oid]
+            `SELECT id FROM public.theo_project_knowledge WHERE project_id = $1`,
+            [activeProjectId]
           );
...
Pass-3 deploys the staged `handlers/theo_message_stream.js` to func-stream via Kudu VFS (overwrite `/site/wwwroot/src/functions/theo_message_stream.js`), GET-back byte-diff, restart. The LIVE baseline (blob `4e71da8`) is retained in `primary-reference/` for rollback. Guardrails: no `reporting_*`; no DB change; other files untouched; modified handler `node --check` clean. Verified via §CURLS. (The package also commits the modified handler + LIVE baseline to `development` for provenance; func-stream is deployed by VFS, not run-from-package.)

## P7 — Risk / regression
- **Live-traffic streaming handler — scoped RAG-context delta.** Confined to the project-knowledge block: by-id lookup + an added access-gate + two project-scoped filters. The owner path is unchanged behavior (an owner classifies non-null on their own project; the search/live filters return the same rows the `created_by` filter did, since the owner IS the project author). Non-fatal try/catch preserved.
- **No leak — the gate is stricter than before for the body.project_id path.** Previously an arbitrary `body.project_id` surfaced only the caller's OWN knowledge (via the `created_by` filter); now it surfaces the project's knowledge ONLY if the caller participates (`theo_project_effective_role` non-null) — so a non-participant gets nothing, and a participant gets exactly their project's knowledge. No path exposes a project's knowledge to a non-member.
- **`searchHistory` untouched** — the caller's own cross-conversation recall stays `created_by = caller`.
- **Rollback is instant.** Re-PUT the retained LIVE baseline via VFS + restart. `node --check` clean; no dependency/config change.
- **Determinism:** authenticated golden curls (§CURLS) assert owner-stream-200 (regression, project knowledge still works) + a no-access project_id → 200 SSE with no injection (non-fatal) + edges.

## P8 — VEP assembly
GCR + Rule Anchors open the pack; P1–P8 walked; Gap Register (G-1 deploy PRE-LAND; G-2 curls PRE-LAND; G-3 NO-GAPS docs); Primary Reference = LIVE handler inlined verbatim + the complete unified diff + staged AFTER handler (`node --check` clean); Kudu VFS §DEPLOY; golden curls §CURLS. Plan-only. On Codex APPROVAL, Claude Code executes Pass-3 (VFS PUT + GET-back diff + restart, run §CURLS), commits to `development`. **This closes G-4 — the shared workspace is at full member/owner parity** (read, continue, attribution, AND project-knowledge context). FE (Phase 2c) is next.
...
5. **Rollback:** PUT the retained LIVE baseline `primary-reference/theo_message_stream.LIVE.js` (blob `4e71da8`) back + restart.

## §CURLS — authenticated golden curls (Claude Code; §5.5)
Auth: `az account get-access-token --resource api://4e1a1e31-5c20-4480-99e4-098901707d9e` (Bearer; never printed). Base `https://vaultgpt-func-stream-cyb4g8bhatddencs.uksouth-01.azurewebsites.net`. Edges (SSE):
- **Owner regression:** `POST theo_message_stream` with a `conversation_id` the caller owns that is linked to a project WITH knowledge → **200** SSE (the turn persists; project knowledge still injected — the access-gate passes for the owner).
- **No-access project_id (safety):** `POST` with an explicit `project_id` the caller does NOT participate in → **200** SSE, no project knowledge injected (the access-gate nulls `activeProjectId`; non-fatal — chat still answers).
- **Edges (regression):** absent conversation_id → 404; bad uuid → 400; unauth → 401.
- **Member parity (new — two-user fixture, manual):** as a project member, `POST` with the owner's PUBLISHED conversation_id (project has knowledge) → **200** SSE with the project's knowledge injected into the member's turn.

**Requested Pass 2 verdict:** Codex APPROVED or REJECTED.

*End of SPW Phase 2b-3e (G-4) member project-knowledge Pass-1 Backend VEP (plan only).*
