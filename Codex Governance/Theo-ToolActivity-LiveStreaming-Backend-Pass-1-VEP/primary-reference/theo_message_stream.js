...
// to decide/continue the tool-loop and to persist the turn. Resolves
// { assistantContent, stopReason, text, citations, model }.
function relayTurnRaw(upstreamRes, stream) {
  return new Promise((resolve) => {
    let buf = "";
    const blocks = {};
    ...
    upstreamRes.on("data", (chunk) => {
      ...
      while ((sep = buf.indexOf("\n\n")) !== -1) {
        ...
        else if (t === "content_block_start") { blocks[d.index] = { type: d.content_block.type, text: "", name: d.content_block.name, id: d.content_block.id, partialJson: "" }; }
        else if (t === "content_block_delta") {
          const b = blocks[d.index] || (blocks[d.index] = { type: "text", text: "", partialJson: "" });
          if (d.delta.type === "text_delta") { b.text += d.delta.text; text += d.delta.text; }
        ...
        else if (t === "message_delta") { if (d.delta && d.delta.stop_reason) stopReason = d.delta.stop_reason; }
      }
    });
    upstreamRes.on("end", () => {
      ...
      resolve({ assistantContent, stopReason, text, citations, model: model || FOUNDRY_DEPLOYMENT });
    });
    upstreamRes.on("error", () => resolve({ assistantContent: [], stopReason: "error", text, citations, model: model || FOUNDRY_DEPLOYMENT }));
  });
}
...
app.http("theo_message_stream", {
  ...
  handler: async (request, context) => {
    ...
    const TOOL_LOOP_MIN_MAX_TOKENS = parsePositiveInt(process.env.THEO_TOOL_LOOP_MAX_TOKENS, 16384);
    ...
    (async () => {
      ...
      try {
        for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
          const { assistantContent, stopReason, text, citations, model: tm } = await relayTurnRaw(upstreamRes, stream);
          finalText += text;
          ...
          for (const tu of toolUses) {
            // Surface the tool call to the client the instant it fires, so the FE can render live
            // agent activity (VA-T7 AgentActivity) — the same clean events the Sigma review agent
            // emits. `input` lets the FE show a compact arg preview; the FE truncates for display.
