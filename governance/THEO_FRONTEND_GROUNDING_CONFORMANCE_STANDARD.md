...
| F-P6 | Repository & active-surface grounding | Confirm target files are on the active surface; no deprecated-code contamination; **no `localStorage`/`sessionStorage`** (1A handover §2.5) **except the Theo Snapshot Storage Exception (Governor item 3, Walter-authorized 2026-07-29 — scoped recents/first-page/self-identity snapshot, per-principal, always revalidated)**; inline-style surface preserved, **no Tailwind/CSS-in-JS conversion in 1A** (1A handover §0.1/§6). | Current state of target files (Read invocations); Claude Code Theo Frontend Governor Standard active-surface and deprecated-code sections. |
...
| VA-T7 | Theo Agent Activity Rendering | `artifacts/theo-agent-activity-reference.jsx` | The Claude-Code-style live agent view for tool-driven agents (first consumer: the Sigma K-1 review agent via `sigma_review_agent_stream`): a collapsible activity panel ABOVE the answer that, while RUNNING, streams the agent's reasoning with a live cursor and lists each tool call the instant it fires (monospace `name(args)` + status dot → ✓/✕); when DONE, collapses to a one-line summary (`✓ Checked … · N tools`, re-expandable) while the tool calls persist as deduped chips on the finished message; the judged answer renders below. Maps the stream events: `delta{thinking}`→reasoning line, `delta{text}`→answer body, `tool`→row + chip, `tool_result`→row status, `done`→collapse. **Also consumed by the general-chat tool-loop** (`theo_message_stream`, DR-T11; first tool `theo_export_spreadsheet` → the VA-T9 download card): the same events (the general-chat `tool` event fires at the tool_use block OPEN carrying `{name}` only — no args — so the tool row is `name()`; the header stays a plain "Thinking…" during the build), PLUS a live **token count** in the header sourced from `event: vault_tokens` (a running CUMULATIVE output-token count, monotonic non-decreasing, tabular-nums) that follows a **two-mode toggle** — SHOWN throughout "processing" (the thinking/planning phase where the model streams its reasoning into the panel, AND the silent tool_use build; the climbing count is the progress signal) and HIDDEN ONLY while the final ANSWER text streams (the answer is the signal), returning at DONE. The general-chat header reads a plain **"Thinking…"** while running — the playful animated status word ("Ruminating…", "Percolating…") lives in the HOST's StatusLine beneath the panel, not this header; the panel label generalises beyond the review context. The reasoning streams in a **fixed-height auto-scrolling region** (the live consumer scrolls it to the newest text on each delta) so the header — verb + live token count — stays PINNED and visible however long the thinking runs, with the newest thinking always in view; `pre-wrap` preserves the summary's paragraph/list breaks. Zero-dependency inline-style (the VA-T1/VA-T5 idiom); no Tailwind, no browser storage. Reproduce faithfully, do not redesign. | CURRENT — landed via Role-C 2026-07-14 (Walter-approved); sha256 verified `50455532c9cebb9bdcc2a46c933fade0c3ed2a6de73edadf5113963cc271e585` (updated 2026-07-18: token source → `event: vault_tokens` cumulative/monotonic; two-mode toggle — count SHOWN through the thinking/planning phase + the build, HIDDEN only while the answer streams; reasoning in a fixed-height auto-scrolling region with pinned header + `pre-wrap`; general-chat header simplified to a plain "Thinking…" while running, with the playful animated status word moved to the host StatusLine below the panel; block-start `{name}`-only general-chat tool rows; reference states C0/C/C2/D — Walter-approved) |
| VA-T8 | Theo Voice Controls | `artifacts/theo-voice-controls-reference.jsx` | The voice controls added to the Theo chat surface: (1) a **dictation** mic button in the composer action row (idle mic → RECORDING tray with an animated waveform + tabular-nums timer + Cancel + "Listening… up to 7:00" + a coral-tinted STOP; on stop the transcript drops into the composer draft for review — nothing is sent automatically; client caps at 7:00), wiring MediaRecorder → `theo_transcribe_audio`; (2) a **read-aloud** control in an actions row under each assistant reply (idle muted "Read aloud" speaker text-button → PLAYING coralDk with a tiny equalizer + "Playing…"; tap to stop; one premium default voice for v1 — the backend accepts an optional `voice`, so a picker is a later FE-only add), wiring `theo_synthesize_speech`. Everything else on the surface is unchanged (VA-T1). Backends DEPLOYED (API Spec §2.11). Zero-dependency inline-style (the VA-T1/VA-T5/VA-T7 idiom); no Tailwind, no browser storage. Reproduce faithfully, do not redesign. | CURRENT — landed via Role-C 2026-07-17 (Walter-approved); evolved via Role-C 2026-07-20 (Walter-approved — the composer mic + send buttons → 40px circular, the mic moved to the right immediately left of send, send carries a bold up-arrow: Claude mobile-composer match) ; sha256 verified `805c4bfb900bd8f6db07ab628b4887dc908d1e70d6de2f0fd1fc40e21f886049` |
...
| VA-T11 | Vault Identity Mark (Spiral of Theodorus) | `vault-origin/public/icon.svg` (canonical source; cross-repo) — implemented byte-verbatim as `src/theo/components/VaultMark.tsx` (static + `building` one-time seed→out assemble) + `src/theo/components/SpiralAssemble.tsx` (the `breathing` build→unbuild→rebuild loop = the "thinking" state), and mirrored in Origin as `../vault-origin/src/shell/VaultMark.tsx` (`colour` / `mono` `currentColor` silhouette) | Theo's canonical brand identity mark — the full-colour Vault logo (Spiral of Theodorus; the deployed app-icon wedges, sand tile omitted → transparent). This mark is Theo's identity wherever a per-product glyph appears: the chat message avatar + build-once greeting hero + sidebar lockup (vault-theo), the active "thinking" status (breathing), and the Origin shell rail tile / Theo tab / "Theo is your home" badge / "Open Theo" buttons / left-panel Theo headers + the login/boot splash (vault-origin). It **supersedes, for Theo's identity, the VA-T1 reference's `Burst` asterisk (L39–57)** and the Origin shell's former lucide `Sparkles` glyph — both retired. VA-T1 otherwise remains CURRENT as the chat-surface reference (only its identity glyph is superseded here). Reproduce faithfully (byte-verbatim `icon.svg` wedges); do not redesign. | CURRENT — landed via Role-C 2026-07-27 (Walter-approved); `icon.svg` sha256 verified `95ef27b9df7882b05d3a47e3a784e2086a63a62800b7ee2df6fd8eb6673e7f3e`; shipped: thinking mark `f731cf2` + Phase-1 chat surface `85308cd` (vault-theo), Phase-2 Origin shell `1c6f3ec` + Phase-3 login splash `b30b8c0` (vault-origin) |
...
| T26 | Direct browser→Anthropic/Foundry model call in productionised code (not via the gateway abstraction), `localStorage`/`sessionStorage` use **(other than the Walter-authorized Theo Snapshot Storage Exception, Governor item 3 — scoped recents/first-page/self-identity snapshot, which does NOT trigger T26)**, Tailwind/CSS-in-JS conversion of the reference surface in 1A, or any change to `corporate-reporting` / `reporting_*` — each contrary to 1A handover §6 guardrails | Claude Code (self-gate), Codex |
