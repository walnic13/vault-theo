...
8. **(Amendment, 2026-08-01 — Walter-directed) Dottie (the L4 governance observer) — implementation model + priority.** Dottie runs on **in-tenant Azure OpenAI** — the existing `Vaultgpt` resource (uksouth; RG `Vault-Tax`; endpoint `https://vaultgpt.openai.azure.com/`), on a **robust GPT-5-class reasoning deployment** (recommended **`gpt-5`**; **`gpt-5-pro`** available for maximum reasoning depth — governance QC is deliberately depth-over-speed, and is on-demand not latency-critical), reached **keyless via Dottie's managed identity** (`Cognitive Services OpenAI User`). This is a **deliberately different model from Theo's Claude** — governance-observer independence (the QC model is not the model that produced the content, so no correlated blind spots). Dottie is a **first-class separate product**: repo **`vault-dottie`**, function app **`vaultgpt-func-dottie`** (shared EP1 plan, SystemAssigned MI, EasyAuth on the shared audience `api://4e1a1e31-5c20-4480-99e4-098901707d9e`), and a **Theo-derived federated-remote FE mounted in the Vault Origin shell** (like DMS / Sigma). **Priority is pulled FORWARD from Stage 6** — this AMENDS the Stage-6 timing in Amendment 2 / §5 / §10: Dottie's **scaffold** (the Azure OpenAI API connection + the FE frame) is built **now, in parallel with Stage 1** (which produces the L1.5 data Dottie will observe); its **observational ruleset** (tag-drift, review-chain integrity, systemic-pattern detection over L1.5/L2/L3) is **fine-tuned as those layers populate** — frame-first, tune-later. UNCHANGED: the Amendment-2 split holds — the security-critical **Tag Guard is already live** (Stage-0 §7.2); Dottie stays the **OBSERVATIONAL** layer and **NEVER reads L1** (Rule 1 inviolable, §7). See §5 / §10. *(BROADENED by Amendment 9 — Dottie is no longer observational-only; the L1 invariant is refined there.)*

9. **(Amendment, 2026-08-01 — Walter-directed) Dottie is a FULL independent agent, not only the L4 observer.** Broadening Amendment 2 / Amendment 8 / §5: Dottie is Theo's **independent governance-and-second-opinion sibling**, on **gpt-5** (a deliberately different model from Theo's Claude — genuine independence, not the same brain twice). It is **MULTI-USE**: (a) a **personal check / second opinion** an individual consults on a substantive issue ("is this position sound? what do you think?") — **NOT only mechanical governance**; and (b) an **embedded process-governance** layer that observes the shared record understanding firm **levels / roles / factors**. Dottie has its **OWN full layered memory** — mirroring THIS architecture with Dottie's own instances, in its own `dottie_*` schema: **Dottie-L1** = a **consensual 1:1 relationship** with the individual (what they share WITH Dottie, their role, patterns, issues raised); **Dottie-L2** = level / role / expectation awareness; **Dottie-L3** = firm / process / governance knowledge. **L1 INVARIANT REFINED (governs §7 Rule 1):** "Dottie never reads L1" means Dottie never reads **Theo's** L1 — the individual's private personal-assistant memory (life, Six-Plates, private notes) stays **private to Theo**. Dottie's own 1:1 layer is **SEPARATE and consensual**; the two personal memories **never cross**. Dottie additionally **OBSERVES** the shared record (Theo's L1.5 / L2 / L3) through the **access-policy engine** (`theo_can_read`, with rights) — it **reads, never duplicates**. **Implementation:** heavily **Theo-derived** (the layered memory architecture + the conversation / streaming backend + the FE) but with **Dottie's own data (`dottie_*`), own model (gpt-5), and a DISTINCT FE identity**. `dottie_ask` (the deployed connection) and all Dottie infra stand. See §5 / §7 (Rule 1) / Amendment 8. **(CORRECTED by Amendment 10 — Dottie-L1 is AUTOMATIC, not consensual.)**

10. **(Amendment, 2026-08-06 — Walter-directed) CORRECTION: personal memory is AUTOMATIC, not consensual — for BOTH Theo and Dottie.** The word **"consensual"** applied to Dottie-L1 in Amendment 9 (and any implied opt-in for a personal memory to *exist*) was an **error**. Neither Theo nor Dottie has consent-gated personal memory: **both distill a personal memory automatically from conversations** (`theo_user_memory` / `dottie_user_memory`), with **NO opt-in-to-exist gate** — the same posture for both agents. Everything else in Amendment 9 stands: Dottie-L1 remains **SEPARATE** from Theo's L1 (the two personal memories never cross; Dottie never reads Theo's L1 — Rule 1 inviolable). Only the *opt-in* qualifier is removed. A future **incognito mode** (a per-user / per-session opt-out) MAY be added as the escape hatch; it is **out of scope now**. This **supersedes the word "consensual" wherever it qualifies Dottie-L1** (Amendment 9, §5, §7 Rule 1) and resolves `DOTTIE_MEMORY_MODEL` open question **O-DOTTIE-L1-CONSENT → automatic**. NOTE: this does NOT touch the separate **Six-Plates life-integration opt-in** (Amendment 3) — that lens is a different concept (whether a user shares *life* signals), unrelated to whether personal memory exists, and is unchanged.
...
- **Dottie (L4 + full independent agent):** watches for tag drift, review-chain integrity, appropriate-access anomalies, and systemic governance patterns; AND (Amendment 9) is a multi-use personal check / second-opinion agent with its own layered memory. Reads Theo's L1.5/L2/L3 (via the engine), never Theo's L1; has its own Dottie-L1 1:1 (automatic — Amendment 10). *(Priority pulled forward from Stage 6; scaffold + FE built now in parallel with Stage 1; the observational ruleset tuned as L1.5/L2/L3 populate. See §A Amendment 8 + Amendment 9.)*
...
1. **L1 is inviolable.** Nothing reads L1 except the individual and their own Personal Theo. Not Dottie, not the firm, not other users. Ever. *(Refined by §A Amendment 9: "Not Dottie" = Dottie never reads **Theo's** L1 — the individual's private personal-assistant memory. Dottie has its OWN separate 1:1 layer (Dottie-L1, automatic — Amendment 10) with the individual; the two personal memories never cross.)*
